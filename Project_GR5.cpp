...
public:
    ...
    {
        // ==========================================================
        //For aesthetic purposes, the underscores '_' used during saving have been changed back to spaces ' ' and printed out.
        // ==========================================================
        string displayCity;
        displayCity = patientAddress.city;
        
        string displayEmName;
        displayEmName = patientContact.emergencyContactName;
        
        string displayStreet;
        displayStreet = patientAddress.street;
        
        StringUtils stringTool;
        int i;
        
        i = 0;
        while(i < stringTool.getLength(displayCity)) { if(displayCity[i] == '_') { displayCity[i] = ' '; } i++; }
        
        i = 0;
        while(i < stringTool.getLength(displayEmName)) { if(displayEmName[i] == '_') { displayEmName[i] = ' '; } i++; }

        i = 0;
        while(i < stringTool.getLength(displayStreet)) { if(displayStreet[i] == '_') { displayStreet[i] = ' '; } i++; }
        // ==========================================================
        ...
        cout << "Street Address  : " << displayStreet << endl;
        cout << "City Location   : " << displayCity << endl;
        cout << "Personal Phone  : " << patientContact.phone << endl;
        cout << "Emergency Name  : " << displayEmName << endl;
...
public:
    ...
    void printDetailedProfile()
    {
        //Change the underscore '_' back to a space ' '
        string displayDept = department;
        string displayPos = position;
        string displayShift = shift;
        
        StringUtils stringTool;
        int i;
        i = 0; while(i < stringTool.getLength(displayDept)) { if(displayDept[i] == '_') { displayDept[i] = ' '; } i++; }
        i = 0; while(i < stringTool.getLength(displayPos)) { if(displayPos[i] == '_') { displayPos[i] = ' '; } i++; }
        i = 0; while(i < stringTool.getLength(displayShift)) { if(displayShift[i] == '_') { displayShift[i] = ' '; } i++; }
        
        cout << "---------- STAFF DETAILED PROFILE ----------" << endl;
        cout << "ID Number          : " << staffID << endl;
        cout << "Assigned Department: " << displayDept << endl;
        cout << "Job Position       : " << displayPos << endl;
        cout << "Work Shift Schedule: " << displayShift << endl;
...
class SymptomChecker
{
	...
	private:
    // ---Safe character reading function---
    char getSafeChar()
    {
        char c;
        cin >> c;
        cin.ignore(10000, '\n'); // Clear all the extra English letters at the end to prevent skipping to the next question!
        return c;
    }
...
public:
    ...
    {
        string name;
        name = "General_Checkup/Undiagnosed"; // Default
        ...
        else if (code == 43) { name = "Anorexia_or_Extreme_Stress"; }
...
public:
    ...
    void saveMainDatabase()
    {
        ...
        while (i < globalApptCount)
        {
            ...
            outFile << saveID << " " << saveName << " " << saveDisease << " " << saveBill << " " << globalApptArray[i].getRecordStatus() << endl;
    ...
    void loadMainDatabase()
    {
        ...
        if (inFile.is_open() == 1)
        {
            ...
            while (globalHospitalQueue.checkEmpty() == 0)
            {
                try { globalHospitalQueue.serveSilent(); }
                catch (...) { break; }
            }
            ...
            while (inFile >> loadID >> loadName >> loadDisease >> loadBill >> loadStatus)
            {
                ...
                if (globalApptCount < MAX_DATABASE_SIZE)
                {
                    ...
                    loadedAppt.setRecordStatus(loadStatus);
                    if (loadStatus == "Active") {
                        globalHospitalQueue.appendToQueue(loadedAppt);
                    }
    ...
    void editRecordDetails(int targetID, string newDisease, double newBill)
    {
        ...
        if (foundIndex != -1)
        {
            ...
            globalApptArray[foundIndex].apptBilling.consultationFee = newBill * 0.5;
            globalApptArray[foundIndex].apptBilling.medicineFee = newBill * 0.3;
            globalApptArray[foundIndex].apptBilling.roomFee = newBill * 0.2;
            globalApptArray[foundIndex].apptBilling.taxAmount = newBill * 0.06;
            globalApptArray[foundIndex].apptBilling.totalAmountDue = newBill + globalApptArray[foundIndex].apptBilling.taxAmount;
            // Sync database
            saveMainDatabase();
...
void runCustomerModule(string currentLoggedInUser)
{
    ...
    StringUtils stringTool;
     ...
     // ========================================================
    //Read the user's detailed information from a text file.
    // ========================================================
    ifstream profileIn;
    profileIn.open("patient_profiles.txt");
    
    if (profileIn.is_open() == 1)
    {
        string f_user, f_blood, f_street, f_city, f_state, f_phone, f_email, f_emName, f_emPhone;
        int f_age, f_zip;
        double f_height, f_weight;
        
        // Read line by line until the end of the file
        while (profileIn >> f_user >> f_age >> f_blood >> f_height >> f_weight 
                         >> f_street >> f_city >> f_state >> f_zip 
                         >> f_phone >> f_email >> f_emName >> f_emPhone)
        {
            // If this line you read is the user's own account
            int isMatch = stringTool.compareStrings(f_user, currentLoggedInUser);
            if (isMatch == 1)
            {
                currentPatient.setAge(f_age);
                currentPatient.setBloodType(f_blood);
                currentPatient.setHeight(f_height);
                currentPatient.setWeight(f_weight);
                currentPatient.setStreet(f_street);
                currentPatient.setCity(f_city);
                currentPatient.setState(f_state);
                currentPatient.setZipCode(f_zip);
                currentPatient.setPhone(f_phone);
                currentPatient.setEmail(f_email);
                currentPatient.setEmergencyContactName(f_emName);
                currentPatient.setEmergencyContactPhone(f_emPhone);
                break; // Once you find it, you don't need to keep looking.
            }
        }
        profileIn.close();
    }
    ...
    while (keepRunningPatientModule == 1)
    {
        ...
        try
        {
            if (menuChoice == 1)
            {
            	...
            	if (globalApptCount >= MAX_DATABASE_SIZE) {
                cout << ">> CRITICAL ERROR: Database is full! (Limit 500)" << endl;
                continue;
            }
            ...
            else if (menuChoice == 2)
            {
                ...
                // ==========================================
                // ---Uniqueness check for manually entered IDs---
                int k = 0;
                while (k < globalApptCount) {
                    if (globalApptArray[k].getApptID() == manualID) {
                        cout << ">> Error: ID already exists! Please choose another ID." << endl;
                        throw InvalidIDException(); 
                    }
                    k = k + 1;
                ...
                if (manualDisease == "") { manualDisease = "None"; }
                
                unsigned int j = 0;
                while (j < stringTool.getLength(manualDisease)) {
                    if (manualDisease[j] == ' ') {
                        manualDisease[j] = '_';
                    }
                    j = j + 1;
                ...
                cin >> manualBill;
                 ...
                 // ==========================================
                // ---Prevents system crashes caused by typing English letters.---
                if (cin.fail())
                {
                    cin.clear();
                    cin.ignore(10000, '\n');
                    cout << ">> Error: Invalid number format!" << endl;
                    throw NegativeBillException(); // Use this exception to safely interrupt the operation.
                }
                // ==========================================
            ...
            else if (menuChoice == 5)
            {
                ...
                // Calling the function directly will automatically print the profile information loaded upon login.
...
void runAdminModule(string currentLoggedInStaff)
{
    ...
    // ========================================================
    // Read the employee's detailed information from the text file.
    // ========================================================
    ifstream staffProfileIn;
    staffProfileIn.open("staff_profiles.txt");
    
    if (staffProfileIn.is_open() == 1)
    {
        string f_user, f_dept, f_pos, f_shift;
        double f_salary;
        int f_clearance;
        
        while (staffProfileIn >> f_user >> f_dept >> f_pos >> f_shift >> f_salary >> f_clearance)
        {
            int isMatch = stringTool.compareStrings(f_user, currentLoggedInStaff);
            if (isMatch == 1)
            {
                currentAdmin.setDepartment(f_dept);
                currentAdmin.setPosition(f_pos);
                currentAdmin.setShift(f_shift);
                currentAdmin.setSalary(f_salary);
                currentAdmin.setClearanceLevel(f_clearance);
                break;
            }
        }
        staffProfileIn.close();
    }
    // ========================================================
        ...
        if (menuChoice == 1)
            {
                ...
                int isValidPatient = 0;
                Appointment servedAppt;
                
                // Added `while` loop: used to filter out ghost patients who have already been "deleted".
                while (isValidPatient == 0)
                {
                    // Retrieve people from the queuing system (if there are truly no people, a QueueEmptyException error will be automatically thrown).
                    servedAppt = globalHospitalQueue.serveFromQueue();
                    
                    int servedID = servedAppt.getApptID();
                    int i = 0;
                    
                    // Check the data in the latest database (Array).
                    while (i < globalApptCount)
                    {
                        if (globalApptArray[i].getApptID() == servedID)
                        {
                            //This update will be updated to reflect the latest information (to prevent situations where the Admin has just modified disease or billing data).
                            servedAppt = globalApptArray[i];
                            
                            // Change the status to "Completed".
                            globalApptArray[i].setRecordStatus("Completed");
                            isValidPatient = 1; // Once confirmed to be a real patient, they can leave the loop.
                            break;
                        }
                        i = i + 1;
                    }
                    // If isValidPatient is still 0, it means that he/she has been deleted by Admin, and the loop will automatically grab the next patient!
                }
           ...
           {
            if (globalApptCount == 0) throw DatabaseEmptyException();

            cout << "\n--- Sorting Database ---" << endl;
            cout << "1. Sort by Appointment ID" << endl;
            cout << "2. Sort by Bill Amount" << endl;
            cout << "3. Sort by Patient Name" << endl;
            ...
            if (sortChoice == 2) {
            algoSys.performBubbleSortByBill(globalApptArray, globalApptCount);
            cout << ">> Sorted by Bill Amount." << endl;
            }
			
			else if (sortChoice == 3) {
            algoSys.performBubbleSortByName(globalApptArray, globalApptCount);
            cout << ">> Sorted by Patient Name." << endl;
            } 
			else {
            algoSys.performBubbleSort(globalApptArray, globalApptCount);
            cout << ">> Sorted by Appointment ID." << endl;
            }

    dbManager.saveMainDatabase(); // Archive after sorting
           ...
           dbManager.logSystemAction("Admin sorted the database array.");
            ...
            else if (menuChoice == 4)
            {
                ...
                if(cin.fail()){
                    ...
                    continue; //Jump directly back to the menu
                }
            ...
            else if (menuChoice == 6)
            {
                ...
                // ==========================================
                // ---Force whitespace to be replaced with underscores to protect TXT files.---
                cin.ignore(10000, '\n'); //First clear the buffer.
                getline(cin, newDiseaseInfo); //The `getline` function allows reading the entire line (including spaces).
                if (newDiseaseInfo == "") { newDiseaseInfo = "None"; }
                
                int j = 0;
                while (j < stringTool.getLength(newDiseaseInfo)) {
                    if (newDiseaseInfo[j] == ' ') {
                        newDiseaseInfo[j] = '_';
                    }
                    j = j + 1;
                ...
                // ==========================================
                // ---Prevent system crashes caused by Admin typing letters while editing---
                if (cin.fail())
                {
                    cin.clear();
                    cin.ignore(10000, '\n');
            ...
            {
                //Directly call the function to print the personalized employee information read during login.
...
if (logChoice == 2)
  ...
  // --- REGISTRATION LOGIC ---
            ...
            // ========================================================
            // 1.Logic for filling in detailed patient information
            // ========================================================
            int isPatientFile;
            isPatientFile = stringTool.compareStrings(authFileName, "patient_credentials.txt");
            
            if (isPatientFile == 1)
            {
                cout << "\n--- Please fill in your Detailed Profile ---" << endl;
                cout << "(You can use spaces for Street, City, State, and Name)" << endl;
                
                int p_age, p_zip;
                double p_height, p_weight;
                string p_blood, p_street, p_city, p_state, p_phone, p_email, p_emName, p_emPhone;
                ...
                cin.ignore(10000, '\n'); 
                p_age = 0; 
                cout << ">> Invalid! Set to 0\n"; 
                }

                int bloodChoice;
                bloodChoice = 0;
                
                while (bloodChoice < 1 || bloodChoice > 8)
                {
                    cout << "\n--- Select your Blood Type ---" << endl;
                    cout << "1. A+    2. A-" << endl;
                    cout << "3. B+    4. B-" << endl;
                    cout << "5. AB+   6. AB-" << endl;
                    cout << "7. O+    8. O-" << endl;
                    ...
                    if (bloodChoice == 1) { p_blood = "A+"; }
                    else if (bloodChoice == 2) { p_blood = "A-"; }
                    else if (bloodChoice == 3) { p_blood = "B+"; }
                    else if (bloodChoice == 4) { p_blood = "B-"; }
                    else if (bloodChoice == 5) { p_blood = "AB+"; }
                    else if (bloodChoice == 6) { p_blood = "AB-"; }
                    else if (bloodChoice == 7) { p_blood = "O+"; }
                    else if (bloodChoice == 8) { p_blood = "O-"; }
                    else { cout << ">> Invalid choice. Please enter a number from 1 to 8." << endl; }
                }
                
                // ==========================================
                // ---Numerical error prevention during registration protects TXT files from corruption.---
                cout << "\nHeight (cm)       : "; cin >> p_height;
                if(cin.fail()) { cin.clear(); cin.ignore(10000, '\n'); p_height = 0.0; cout << ">> Invalid! Set to 0.0\n"; }
                
                cout << "Weight (kg)       : "; cin >> p_weight;
                if(cin.fail()) 
                { 
                    cin.clear(); 
                    cin.ignore(10000, '\n'); 
                    p_weight = 0.0; 
                    cout << ">> Invalid! Set to 0.0\n"; 
                } 
                else 
                {
                    cin.ignore(10000, '\n'); // Only clear the newline character here if the operation is successful.
                }

                cout << "Street Address    : "; getline(cin, p_street);
                if (p_street == "") { p_street = "None"; }
                
                cout << "City              : "; getline(cin, p_city);
                if (p_city == "") { p_city = "None"; }

                cout << "State             : "; getline(cin, p_state);
                if (p_state == "") { p_state = "None"; }
                cout << "Emergency Name    : "; getline(cin, p_emName);
                if (p_emName == "") { p_emName = "None"; }
                
                cout << "Zip Code          : "; cin >> p_zip;
                if(cin.fail()) { cin.clear(); cin.ignore(10000, '\n'); p_zip = 0; cout << ">> Invalid! Set to 0\n"; }
                
                cout << "Phone Number      : "; cin >> p_phone;
                cout << "Email Address     : "; cin >> p_email;
                cout << "Emergency Phone   : "; cin >> p_emPhone;
                // ======================================
                // Replace spaces with underscores '_'
                int i;
                i = 0; while(i < stringTool.getLength(p_street)) { if(p_street[i] == ' ') { p_street[i] = '_'; } i++; }
                i = 0; while(i < stringTool.getLength(p_city)) { if(p_city[i] == ' ') { p_city[i] = '_'; } i++; }
                i = 0; while(i < stringTool.getLength(p_state)) { if(p_state[i] == ' ') { p_state[i] = '_'; } i++; }
                i = 0; while(i < stringTool.getLength(p_emName)) { if(p_emName[i] == ' ') { p_emName[i] = '_'; } i++; }
                
                ofstream profileOut;
                profileOut.open("patient_profiles.txt", ios::app);
                profileOut << inputUsername << " " << p_age << " " << p_blood << " " 
                           << p_height << " " << p_weight << " "
                           << p_street << " " << p_city << " " << p_state << " " << p_zip << " " 
                           << p_phone << " " << p_email << " " << p_emName << " " << p_emPhone << endl;
                profileOut.close();
            }
            
            // ========================================================
            // 2.Logic for filling in detailed employee information
            // ========================================================
            int isStaffFile;
            isStaffFile = stringTool.compareStrings(authFileName, "staff_credentials.txt");
            
            if (isStaffFile == 1)
            {
                cout << "\n--- Please fill in your Staff Profile ---" << endl;
                cout << "(You can use spaces for text fields)" << endl;
                
                string s_dept, s_pos, s_shift;
                double s_salary;
                int s_clearance;
                
                cin.ignore(10000, '\n'); // Clear the newline characters left by the password.
                
                cout << "Department (e.g. ICU) : "; getline(cin, s_dept);
                if (s_dept == "") { s_dept = "None"; }
                
                cout << "Job Position          : "; getline(cin, s_pos);
                if (s_pos == "") { s_pos = "None"; }
                
                cout << "Work Shift            : "; getline(cin, s_shift);
                if (s_shift == "") { s_shift = "None"; } 
                
                cout << "Monthly Salary ($)    : "; cin >> s_salary;
                cout << "Security Clearance (1-5): "; cin >> s_clearance;
                
                //Replace spaces with underscores '_'
                int i;
                i = 0; while(i < stringTool.getLength(s_dept)) { if(s_dept[i] == ' ') { s_dept[i] = '_'; } i++; }
                i = 0; while(i < stringTool.getLength(s_pos)) { if(s_pos[i] == ' ') { s_pos[i] = '_'; } i++; }
                i = 0; while(i < stringTool.getLength(s_shift)) { if(s_shift[i] == ' ') { s_shift[i] = '_'; } i++; }
                
                ofstream staffProfileOut;
                staffProfileOut.open("staff_profiles.txt", ios::app);
                
                staffProfileOut << inputUsername << " " << s_dept << " " << s_pos << " " 
                                << s_shift << " " << s_salary << " " << s_clearance << endl;
                                
                staffProfileOut.close();
            }
            // ========================================================
